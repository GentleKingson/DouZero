...
| Deployment selection | `DeepAgent.act(infoset)` | P04 resolved (legacy_factorized backend). P05 resolved: `DeepAgentV2` (public-only, `backend='v2'`) with `act_v2(ObservationV2)` + legacy-compatible `act(infoset)`. Strict manifest-bearing loader → P16. |
