...
The model-optimization work started at
`22a891cb98212e788d1e77a6c5a92bc21e1847d0`; all C0 commits below that point
remain unchanged. A1 remains the production topology and C0 remains an
experimental comparison target.
...
Two complete formal 8-actor/48-buffer A1 runs at commit `94a2fec` measured
`12,632.96` and `12,729.28` frames/s, with `13,023.60` and `13,026.43`
decisions/s. Both reported CUDA available, zero live learner threads at exit,
and completed checkpoint output. The third repeat was killed externally at
startup on this host, so this is diagnostic evidence rather than a promotable
three-repeat result.
